...
node1 = queue.Queue(q_max)
node2 = queue.Queue(q_max)

edge1 = queue.Queue(q_max)
edge2 = queue.Queue(q_max)
...
f = open("C:/Users/강지완/Desktop/DRL/test3.txt", "w")
...
def img_mod():
    ...
    while True:
        ...
        #print("Buff Out : "+str(time.time()))
        nodeRewardAfterBuff.put(time.time())
...
def train(q, q_target, memory, optimizer):
    ...
    ###batch_size 만큼 memory에서 랜덤하게 꺼내서 저장 후 형변환
...
def judge_algo():
    ...
    q = Qnet()
    q_target = Qnet()
    q_target.load_state_dict(q.state_dict())  # q 에서 q_target 으로 매개변수나 상태 복제
    memory = ReplayBuffer()
    ...
    ### 2. 학습에 쓸 state, action, reward, state_prime(다음 state)를 저장할 배열 선언
    ...
    while(cam.isOpened()):
       ...
       # edge_latency = edge_time * (edgeBeforeBuff.qsize() + 1)
        ...
        ### 3. local optima를 방지하기 위한 값 설정. rand 함수 돌려서 이거보다 작으면 반대 행동 취하기
        epsilon = max(0.01, 0.99 - 0.01 * (num / 200))

        ### 4. state 배열에 현재 상태 저장
        ...
        if a == 0:
            ...
            # if r != 0:
                ...
                #print("now q : ", q1)
                r_count += 1
            #다음 state에 자기 자신을 buff에 넣었을 때의 size들 저장
        ...
        ### 8. 만약 뽑을 reward 값이 있다면
        if r_count != 0 and r_count2 != 0:
            #어느쪽의 reward를 뽑는지에 따라 어느쪽 reward 수를 -1 할 지 결정
            if procSeq2.get() == 1:
                r_count -= 1
            if procSeq2.get() == 2:
                r_count2 -= 1
            ...
            #메모리에 샘플 넣었으니 train 함수 호출.271라인으로 이동
            train(q, q_target, memory, optimizer)
            #학습한 상태의 q를 q_target 으로 복사.
